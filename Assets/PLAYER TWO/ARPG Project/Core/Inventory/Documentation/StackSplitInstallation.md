...
4. Assign these references on the prefab's `GUIStackSplitMenu` component:
   - `Decrease Button` for the `-` control
   - `Increase Button` for the `+` control
   - `Confirm Button` for the confirmation control
   - `Amount Text` for the TextMeshPro quantity display
5. Select the scene object with the main `GUI` component.
6. Assign your prefab to `Stack Split Menu Prefab`.
7. Optionally assign `Stack Split Menu Container` if the menu should appear under a specific canvas/container instead of directly under the `GUI` transform.

The custom prefab still uses the same split logic, button state updates, and min/max quantity limits, so the duplication safeguards remain active.
