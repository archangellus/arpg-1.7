...
## Notes for Custom UI Styling

The split menu is generated at runtime by `GUIStackSplitMenu.Create(...)`. To customize its look, edit:
...
- panel size and color in `Create(...)`
- button positions and labels in `BuildControls(...)`
- button colors in `CreateButton(...)`
- quantity text size/color in `CreateText(...)`

If you prefer a prefab-based UI, you can replace the runtime creation path with a serialized prefab reference, but keep the same source/destination stack bounds logic to prevent duplication.
