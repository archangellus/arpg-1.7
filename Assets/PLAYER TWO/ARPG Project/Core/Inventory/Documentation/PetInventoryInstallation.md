...
- Right-click an equippable, consumable, or skill item inside the pet inventory to equip/use/learn it directly on the player when possible.
- Right-click other pet inventory items, or pet items that cannot currently be equipped/used/learned, to move them into the player inventory when space is available.
