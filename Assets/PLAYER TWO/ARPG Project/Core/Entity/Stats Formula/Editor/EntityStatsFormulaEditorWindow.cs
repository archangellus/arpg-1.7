...
namespace PLAYERTWO.ARPGProjectEditorTools
{
    ...
    class StatsFormulaGraphView : GraphView
    {
        ...
        void SetupContextMenu()
        {
            ...
            {
                var position = GetContentPositionFromScreen(context.screenMousePosition);
                var port = context.target as Port;
                ShowNodeCreationMenu(position, port);
            };
        }
        ...
        void OnPortMouseDown(MouseDownEvent evt, Port port)
        {
            if (evt.button == 0)
                m_draggedPort = port;
        }

        void OnGraphMouseUp(MouseUpEvent evt)
        {
            if (m_draggedPort == null || evt.button != 0)
                return;

            var draggedPort = m_draggedPort;
            m_draggedPort = null;

            if (IsEventTargetPort(evt.target as VisualElement))
                return;

            ShowNodeCreationMenu(GetContentPosition(evt.localMousePosition), draggedPort);
        }

        static bool IsEventTargetPort(VisualElement element)
        {
            while (element != null)
            {
                if (element is Port)
        ...
        void AddInput(Node node, string name)
        {
            ...
            port.portName = name;
            port.RegisterCallback<MouseDownEvent>(
                evt => OnPortMouseDown(evt, port),
