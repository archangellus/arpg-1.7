...
namespace PLAYERTWO.ARPGProjectEditorTools
{
    public class EntityStatsFormulaEditorWindow : EditorWindow
    {
        ...
        void BuildWindow()
        {
            ...
            toolbar.Add(new Button(LoadBuiltInExample) { text = "Load Example" });
            toolbar.Add(new Button(LoadAllBuiltInExamples) { text = "Load All Examples" });
        ...
        void LoadBuiltInExample()
        {
            ...
            Undo.RecordObject(m_graphAsset, "Load Built-In Stat Formula Example");
            EntityStatsFormulaExampleBuilder.AddOrReplaceBuiltInExample(m_graphAsset, m_target);
            LoadFormula();
            Save();
        }

        void LoadAllBuiltInExamples()
        {
            if (!EnsureGraphAsset())
                return;

            Undo.RecordObject(m_graphAsset, "Load Built-In Stat Formula Examples");
            EntityStatsFormulaExampleBuilder.AddOrReplaceAllBuiltInExamples(m_graphAsset);
            LoadFormula();
            Save();
        }

        bool EnsureGraphAsset()
        {
            if (m_graphAsset)
                return true;
            ...
            var create = EditorUtility.DisplayDialog(
                ...
                "Create a formula graph asset before loading built-in examples.",
                "Create Graph Asset",
                "Cancel"
            );
