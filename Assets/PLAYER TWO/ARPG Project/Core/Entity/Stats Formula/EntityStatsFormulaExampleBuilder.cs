...
{
    public enum EntityStatsFormulaExampleType
    {
        Player,
        Enemy,
        Boss,
    ...
    public static class EntityStatsFormulaExampleBuilder
    {
        public static void ApplyBuiltInExamples(
            ...
            ApplyBuiltInExamples(graph, EntityStatsFormulaExampleType.Player, replaceExisting);

        public static void ApplyBuiltInExamples(
            EntityStatsFormulaGraph graph,
            EntityStatsFormulaExampleType exampleType,
            bool replaceExisting = true
        )
        {
            if (!graph)
            ...
            foreach (EntityStatsFormulaTarget target in System.Enum.GetValues(typeof(EntityStatsFormulaTarget)))
                ApplyBuiltInExample(graph, target, exampleType);
        }
        ...
        public static void ApplyBuiltInExample(
            ...
            ApplyBuiltInExample(graph, target, EntityStatsFormulaExampleType.Player);
        ...
        )
        {
            ...
            AddOrReplace(graph, CreateBuiltInExample(target, exampleType));
        }

        public static EntityStatsFormulaData CreateBuiltInExample(EntityStatsFormulaTarget target) =>
            CreateBuiltInExample(target, EntityStatsFormulaExampleType.Player);

        public static EntityStatsFormulaData CreateBuiltInExample(
            EntityStatsFormulaTarget target,
            EntityStatsFormulaExampleType exampleType
        )
        {
            ...
            switch (target)
            {
                case EntityStatsFormulaTarget.MinDamage:
                    formula = BuildDamage(target, true);
                    break;
                case EntityStatsFormulaTarget.MaxDamage:
                    formula = BuildDamage(target, false);
                    break;
                case EntityStatsFormulaTarget.MinMagicDamage:
                    formula = BuildMagicDamage(target, true);
                    break;
                case EntityStatsFormulaTarget.MaxMagicDamage:
                    formula = BuildMagicDamage(target, false);
                    break;
                case EntityStatsFormulaTarget.NextLevelExperience:
                    formula = BuildNextLevelExperience();
                    break;
                case EntityStatsFormulaTarget.MaxHealth:
                    formula = BuildMaxHealth();
                    break;
                case EntityStatsFormulaTarget.MaxMana:
                    formula = BuildMaxMana();
                    break;
                case EntityStatsFormulaTarget.AttackSpeed:
                    formula = BuildAttackSpeed();
                    break;
                case EntityStatsFormulaTarget.CriticalChance:
                    formula = BuildCriticalChance();
                    break;
                case EntityStatsFormulaTarget.Defense:
                    formula = BuildDefense();
                    break;
                case EntityStatsFormulaTarget.ChanceToBlock:
                    formula = BuildChanceToBlock();
                    break;
                case EntityStatsFormulaTarget.BlockSpeed:
                    formula = BuildBlockSpeed();
                    break;
                case EntityStatsFormulaTarget.StunChance:
                    formula = BuildStunChance();
                    break;
                case EntityStatsFormulaTarget.StunSpeed:
                    formula = BuildStunSpeed();
                    break;
                case EntityStatsFormulaTarget.Accuracy:
                    formula = BuildAccuracy();
                    break;
                case EntityStatsFormulaTarget.Evasion:
                    formula = BuildEvasion();
                    break;
                default:
                    formula = BuildDamage(EntityStatsFormulaTarget.MinDamage, true);
                    break;
            }

            ApplyExampleTypeMultiplier(formula, exampleType);
            return formula;
        }

        static void ApplyExampleTypeMultiplier(
            EntityStatsFormulaData formula,
            EntityStatsFormulaExampleType exampleType
        )
        {
            var multiplier = GetExampleTypeMultiplier(exampleType, formula.target);
            ...
            formula.connections.Remove(connection);

            var multiplierNode = new EntityStatsFormulaNodeData
            {
                type = EntityStatsFormulaNodeType.Constant,
                constant = multiplier,
                position = new Rect(result.position.x - 260, result.position.y + 120, 170, 92),
            };
            var multiplyNode = new EntityStatsFormulaNodeData
            {
                type = EntityStatsFormulaNodeType.Operator,
                operation = EntityStatsFormulaOperator.Multiply,
                position = new Rect(result.position.x - 260, result.position.y, 120, 92),
            };

            formula.nodes.Add(multiplierNode);
            formula.nodes.Add(multiplyNode);
        ...
        static float GetExampleTypeMultiplier(
            EntityStatsFormulaExampleType exampleType,
            EntityStatsFormulaTarget target
        )
        {
            switch (exampleType)
            {
                case EntityStatsFormulaExampleType.Enemy:
                    return GetEnemyMultiplier(target);
                case EntityStatsFormulaExampleType.Boss:
                    return GetBossMultiplier(target);
                default:
                    return 1f;
            }
        }

        static float GetEnemyMultiplier(EntityStatsFormulaTarget target)
        {
            switch (target)
            {
                case EntityStatsFormulaTarget.MinDamage:
        ...
        static float GetBossMultiplier(EntityStatsFormulaTarget target)
        {
            switch (target)
            {
                case EntityStatsFormulaTarget.MinDamage:
        ...
        public static EntityStatsFormulaData CreateEntityExample(
            EntityStatsFormulaTarget target,
            EntityStatsManager sourceStats
        )
        {
            var formula = CreateBuiltInExample(target);
            ApplyEntityStatsSnapshot(formula, sourceStats);
            return formula;
        }

        static void ApplyEntityStatsSnapshot(
            EntityStatsFormulaData formula,
            EntityStatsManager sourceStats
        )
        {
            ...
            foreach (var node in formula.nodes)
            {
                ...
                if (!TryGetEntityStatsValue(sourceStats, node.input, out var value))
                    continue;
        ...
        static bool TryGetEntityStatsValue(
            EntityStatsManager sourceStats,
