...
Use `Comment` and `Reroute` nodes to document and organize larger formulas. Comments are editor-only documentation nodes, while reroutes pass a value through to improve graph readability.
...
Press `Search Nodes`, right-click and choose `Search/Create Node...`, or drag a connection into empty graph space to open the searchable node creation window. Nodes are grouped into Inputs, Constants, Math, Reusable, and Organization categories, and operator entries include the designer-friendly `To Percent 0-1 / Normalize Percent` option.
