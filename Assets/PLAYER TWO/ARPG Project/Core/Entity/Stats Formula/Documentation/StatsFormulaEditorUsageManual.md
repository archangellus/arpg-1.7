...
## Step 6: Build the Example Formula from the Screenshot
...
## Step 7: Load Built-In Formula Examples

The editor can generate example graphs that are built from the existing `EntityStatsManager` formulas. Use these examples as a starting point before customizing balance values.

### Load an Example for the Selected Stat

1. Create or assign a formula graph asset.
2. Choose a stat in the `Stat` dropdown, such as `MaxHealth` or `Defense`.
3. Click `Load Example`.
4. The current stat formula is replaced with an enabled node graph based on the matching built-in calculation.
5. Click `Save` if the graph was not already saved by the editor.

### Load Examples for Every Supported Stat

1. Create or assign a formula graph asset.
2. Click `Load All Examples`.
3. The graph receives enabled example formulas for every supported formula target.
4. Use the `Stat` dropdown to inspect each generated formula.
5. Customize or disable individual formulas as needed.

### Create a Separate Built-In Examples Asset

1. In the Unity top menu, choose `Tools > PLAYER TWO > ARPG Project > Create Built-In Stats Formula Examples`.
2. Choose where to save the example graph asset.
3. Select the created asset and open the Stats Formula Editor to inspect or edit the generated formulas.

> Note: The examples are intended to mirror the existing formulas as node graphs. Integer stat outputs are rounded by the runtime formula integration when assigned back to `EntityStatsManager`.
