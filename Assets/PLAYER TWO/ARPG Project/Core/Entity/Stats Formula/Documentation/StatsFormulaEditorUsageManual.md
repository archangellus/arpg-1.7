...
Use resizable comment boxes for free-form notes; drag the lower-right handle on a comment to resize it. Use `Group Selection` from the right-click menu to create a titled group box around selected nodes. Reroutes remain available as flow helpers for keeping long wires readable.
...
Right-click the graph and choose `Create Node...`, or drag a connection into empty graph space, to open the searchable node creation window. Nodes are grouped into Inputs, Constants, Math, Reusable, Flow, and Annotations categories, and operator entries include the designer-friendly `To Percent 0-1 / Normalize Percent` option.
...
Use the right-click graph menu for `Create Node...`, `Duplicate`, `Copy`, `Cut`, `Paste`, `Delete`, `Group Selection`, `Auto Layout`, and `Frame All`. The top toolbar intentionally keeps creation/edit controls compact and wraps when the window is narrow. Keyboard shortcuts are supported in the graph view: `Ctrl/Cmd+C` copies, `Ctrl/Cmd+X` cuts, `Ctrl/Cmd+V` pastes, `Ctrl/Cmd+D` duplicates, `Delete` deletes, `Ctrl/Cmd+Z` undoes, and `Ctrl/Cmd+Y` redoes. Auto-layout arranges nodes left-to-right by graph dependency depth, with source inputs/constants on the left and result nodes to the right. Use `Group Selection` to create a titled group box around selected nodes. The graph also includes a minimap and color-coded node headers for inputs/references, constants, operators, results, and annotations.
